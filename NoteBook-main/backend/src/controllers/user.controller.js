...
//   try {
//     const getData = req.body;
//     getData.email = getData.email.trim().toLowerCase();
...
const signup = async (req, res) => {
  try {
    ...
    // Your model already hashes password using pre("save")
...
const resetPassword = asyncErrorHandler(async (req, res) => {
  ...
  await user.save(); // pre("save") will hash password automatically

  res.status(200).json(
    new ApiResponse(200, null, null, "success", "password changed successfully")
  );
});
...
const deleteUserProfile = asyncErrorHandler(async (req, res) => {
  ...
  res.status(200).json(
    new ApiResponse(200, null, null, "success", "account deleted successfully")
  );
});
