...
/** ApiResponse body: { status, data: { analysis }, token, ... } */
function extractApiStringField(res, fieldName) {
  const root = res?.data;
  if (!root || typeof root !== "object") return null;
  const inner = root.data;
  if (inner && typeof inner === "object" && !Array.isArray(inner)) {
    const v = inner[fieldName];
    if (typeof v === "string" && v.trim()) return v;
  }
  const top = root[fieldName];
  if (typeof top === "string" && top.trim()) return top;
  return null;
}

const AI_REQUEST_OPTS = {
  ...
  const handleAISummarizer = async (noteContent) => {
    ...
    try {
      const response = await axios.post(
        apiRoutes.summarizeURI,
        { noteContent },
        {
          ...AI_REQUEST_OPTS,
          headers: {
            ...AI_REQUEST_OPTS.headers,
            Authorization: `Bearer ${token}`,
          },
        }
      );
      const analysis = extractApiStringField(response, "analysis");
      setAiSummary(analysis || "No summary was returned.");
    } catch (err) {
      const message =
        err.code === "ECONNABORTED"
          ? "Request timed out. Your note may be very long or the AI service is slow — try again."
          : err.response?.data?.message ||
            err.message ||
            "Could not generate a summary. Check your connection and try again.";
      ...
      <div className="mb-4 overflow-y-auto flex-1 space-y-4">
        ...
        <div>
          ...
          <p className="text-gray-800 bg-blue-50 p-3 rounded whitespace-pre-wrap break-words">
            {aiSummaryLoading ? "Generating summary…" : aiSummary || "—"}
          </p>
        </div>
      </div>
